...
p0_count: 1
p1_count: 10
target_identity: "file:C:\\Users\\iago.luchtenberg\\Documents\\Sistema - Iago\\Gestor de Peças - Area de Testes\\web"
timestamp: 2026-09-24T17-21-53Z
slug: web
---
...
| IagoDev / Dev Observatory | admin real | ❌ | ❌ | ❌ | ❌ | ❌ | ❌ | ❌ | ❌ | ver §6 |
...
|---|---|---|---|
| Firefox e WebKit | Só o Chromium está instalado (`ms-playwright/chromium-1234`) | Verifiquei a instalação; o `playwright install firefox webkit` é um download e exige a sua autorização | Autorizar `C:/Python314/python.exe -m playwright install firefox webkit` e rodar de novo `scripts/matrix.py` e `scripts/flows*.py` |
| Dev Observatory / IagoDev real (8001) | Exige login com credenciais reais na instância do usuário; não digito senhas reais | `GET /api/v1/dev-observatory` na 8001 devolveu o fallback do SPA (1059 bytes, CSP `default-src 'none'`), o que indica a flag desligada | Você fazer login na 8001 (ou habilitar a flag no harness TEST) e eu rodar a matriz naquela rota |
| Leitor de tela real (NVDA/Narrator) | Não há leitor controlável no ambiente headless | Substituído pela árvore de acessibilidade e pelas checagens de nome/papel/valor | Uma sessão manual com o NVDA |
| Toque físico e luva | Só dá para emular (`has_touch`) | Medi os alvos com toque emulado | Teste em tablet ou tela de chão de fábrica |
...
**Com esses itens em aberto, a cobertura não é 100%.**
...
A **gestão** já parece um produto: mesma sidebar, abas, barra de filtros, estados de erro e tokens de cor, sem overflow em nenhuma das 37 rotas. O **operador**, o **Andon** e a **administração** falam outras línguas:
